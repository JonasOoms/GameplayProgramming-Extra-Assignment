...
{
	//Create Rigidbody
	const Elite::RigidBodyDefine define = Elite::RigidBodyDefine(0.01f, 0.1f, Elite::ERigidBodyType::eDynamic, false);
...
{
	const float o = GetRotation();
	const Elite::Vector2 p = GetPosition();
	constexpr float r = Elite::ToRadians(150.f);
