...
void App_BT::Start()
{
	...
	BehaviorTree* const pBehaviorTree = new BehaviorTree(pBlackboard, new BehaviorAction(BT_Actions::Patrol));
...
void App_BT::Render(float deltaTime) const
{
	...
	{
		for (const Elite::Vector2& pathPoint : m_PatrolPath)
			DEBUGRENDERER2D->DrawCircle(pathPoint, 2.0f, Color(0.f, 0.f, 1.f), -0.2f);

		//DEBUGRENDERER2D->DrawSegment(m_pAgent->GetPosition(), m_vPath[0], Color(1.f, 0.0f, 0.0f), -0.2f);
		const int size = static_cast<int>(m_PatrolPath.size());
		for (int i = 0; i < size; i++)
		{
...
Elite::Blackboard* App_BT::CreateBlackboard(SmartAgent* const smartAgent)
{
