...
void Camera2D::Update()
{
	...
	{
		const Elite::MouseData& mouseData = INPUTMANAGER->GetMouseData(Elite::InputType::eMouseMotion);
		const Elite::Vector2 pos = ConvertScreenToWorld(Elite::Vector2(float(mouseData.X), float(mouseData.Y)));
	...
	{
		const Elite::MouseData& mouseData = INPUTMANAGER->GetMouseData(Elite::InputType::eMouseWheel);
		if (mouseData.Y > 0)
			m_zoom /= 1.1f;
