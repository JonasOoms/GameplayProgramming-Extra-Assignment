...
namespace Elite
{
	...
	struct Vertex final
	{
		...
		*/
		...
		//=== Constructors ===
		Vertex(const Elite::Vector2& _position, float depth, const Elite::Color& _color, float _size = 1.0f) :
			position(_position.x, _position.y, depth), color(_color), size(_size)
		{}

		Vertex(const Elite::Vector2& _position, const Elite::Color& _color, float _size = 1.0f) :
			position(_position.x, _position.y, -0.5f), color(_color), size(_size)
		{}

		Vertex(const Elite::Vector3& _position, const Elite::Color& _color, float _size = 1.0f) :
