	...
	public:
		...
		static std::vector<Elite::Vector2> OptimizePortals(const std::vector<Portal>& portals)
		{
			...
			for (unsigned int portalIdx = 1; portalIdx < amtPortals; ++portalIdx)
			{
				...
				{
					float crossOld = Elite::Cross(rightLeg, leftLeg);
					float crossNew = Elite::Cross(newRightLeg, leftLeg);

					
					
					if (crossOld * crossNew > 0)
					{
						// we dont cross over
				...
				if (Elite::Cross(newLeftLeg, leftLeg) > 0)
				{
					float crossOld = Elite::Cross(leftLeg, rightLeg);
					float crossNew = Elite::Cross(newLeftLeg, rightLeg);
			...
			vPath.emplace_back(portals[amtPortals - 1].Line.p1);
