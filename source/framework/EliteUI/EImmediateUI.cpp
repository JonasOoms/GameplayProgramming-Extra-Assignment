...
void Elite::EImmediateUI::Initialize(EliteRawWindow const pWindow)
{
	...
	SDL_bool succeeded = SDL_GetWindowWMInfo(pWindow, &wmInfo);
	if (!succeeded)
		return;
...
void Elite::EImmediateUI::StaticRender(ImDrawData* const drawData)
{
...
void Elite::EImmediateUI::NewFrame(EliteRawWindow const pWindow, float deltaTime)
{
	ImGuiIO& io = ImGui::GetIO();
...
void Elite::EImmediateUI::SetClipboardText(const char* const text)
{
