...
int main(int argc, char* argv[])
{
	...
	ENABLE_LEAK_REPORT; // This gets automically disabled for release
	//DEBUG_BREAK_ON_ALLOCATION_ID(1363); // This is how you can find the leaks

	// Random seeding: uncomment following line
	//srand(unsigned int(time(nullptr)));

	// Parse incoming supported arguments
	int x{ -1 };
	int y{ -1 };
	if (argc > 1)
	{
	...
	try
	{
		...
		// Specify window position (if we got the arguments)
		if (x != -1 && y != -1)
		...
		while (!pWindow->ShutdownRequested())
		{
			//Timer
			TIMER->Update();
			const float elapsed = TIMER->GetElapsed();
