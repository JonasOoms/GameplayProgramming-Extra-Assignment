...
struct EPhysicsCircleShape final : EPhysicsShape
{
	Elite::Vector2 position = Elite::ZeroVector2;
	float radius = 1.f;
	
	EPhysicsCircleShape() { type = EPhysicsShapeType::CircleShape; };
	EPhysicsCircleShape(const Elite::Vector2& _position, float _radius):position(_position),radius(_radius){ type = EPhysicsShapeType::CircleShape; }
};

struct EPhysicsBoxShape final : EPhysicsShape
{
	Elite::Vector2 position = Elite::ZeroVector2;
	float width = 1.f;
	float height = 1.f;
	...
	EPhysicsBoxShape(const Elite::Vector2& _position, float _width, float _height, float _angle = 0.f)
		...
		type = EPhysicsShapeType::BoxShape;
	}
};

struct EPhysicsPolygonShape final : EPhysicsShape
{
