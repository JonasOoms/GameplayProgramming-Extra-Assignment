...
namespace Elite 
{
	...
	public:
		...
		void MatrixMultiply(const FMatrix& op2, FMatrix& result) const
		{
		...
		{
			const int mR = min(GetNrOfRows(), op2.GetNrOfRows());
			const int mC = min(GetNrOfColumns(), op2.GetNrOfColumns());
