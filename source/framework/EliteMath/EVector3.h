...
namespace Elite {
	...
	struct Vector3 final
	{
		...
		inline bool operator==(const Vector3& v) const /*Check if both components are equal*/
		{ return AreEqual(x, v.x) && AreEqual(y, v.y) && AreEqual(z, v.z); }
		inline bool operator!=(const Vector3& v) const /*Check if one or both components are NOT equal*/
		{ return !(*this == v);	}
		...
		inline Vector3 Reject(const Vector3& v) const
...
#pragma region GlobalVectorFunctions
	...
	inline Vector3 GetAbs(const Vector3& v)
	{ return v.GetAbs(); }

	inline Vector3 Abs(Vector3& v) /*! Make absolute Vector3 of this Vector3 */
	{ v = v.GetAbs(); }
