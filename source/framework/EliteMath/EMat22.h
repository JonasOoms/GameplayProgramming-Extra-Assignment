...
namespace Elite {
	...
	struct Mat22 final
	{
		...
		inline Mat22 operator-(const Mat22& m) const
		{ return Mat22(r[0] - m.r[0], r[1] - m.r[1]); }
		inline Mat22 operator*(float scale) const
		...
		inline Mat22& operator-=(const Mat22& m)
		{ r[0] -= m.r[0]; r[1] -= m.r[1]; return *this;	}
		inline Mat22& operator*=(float scale)
		{ r[0] *= scale; r[1] *= scale; return *this; }
		inline Mat22& operator*=(const Mat22& m)
		{ Mat22 result = *this * m; return *this = result; }

		//=== Internal Vector Functions ===
		void SetIdentity()
		{ r[0] = Vector2( 1.f, 0.f); r[1] = Vector2(0.f, 1.f); }
		float Determinant() const //Formula: x1*y2 - y1*x2
		...
		Mat22 Inverse() const
		{
			...
			det = 1.f / det;
			const float m00 = det * r[1][1], m01 = det * (-r[0][1]);
			const float m10 = det * (-r[1][0]), m11 = det * r[0][0];
			return Mat22(m00, m01, m10, m11);
		}
	};

	//=== Global Matrix Functions ===
	inline float GetDeterminant(const Mat22& m)
	{ return m.Determinant(); }

	inline Mat22 GetInverse(const Mat22& m)
	{ return m.Inverse(); }
}
#endif
